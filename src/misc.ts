 ...
 */
export function linkBtnToCallback(
  buttonId: string,
...
export async function mintTokens(
  ...
  mintAmount: BigNumber = DEFAULT_MINT_AMOUNT
): Promise<string> {
  ...
  const mintAmountInBaseUnits = Web3Wrapper.toBaseUnitAmount(
    mintAmount,
    numDecimals.toNumber()
  );

  const tx = await contractInstance
    .mint(mintAmountInBaseUnits)
