...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  providerEngine.addProvider(
    new MetamaskSubprovider(scopedWindow.web3.currentProvider)
  );
  ...
  if (chainId !== 42) {
    throw new Error(
      `Chain ID should be set to Kovan, it was set to ${chainId}`
    );
  }
  ...
  linkBtnToCallback('mintDAI', () =>
    mintTokens(account, FAKE_DAI, provider, DEFAULT_MINT_AMOUNT)
  );
  linkBtnToCallback('mintUSDC', () =>
    mintTokens(account, FAKE_USDC, provider, DEFAULT_MINT_AMOUNT)
  );
  linkBtnToCallback('swapDaiForUsdc', () =>
    performSwapAsync(usdcToken, daiToken, 100, account, provider)
  );
  linkBtnToCallback('swapUsdcForDai', () =>
    performSwapAsync(daiToken, usdcToken, 100, account, provider)
  );
  ...
  setInterval(async () => {
    ...
    const daiAllowance = await daiToken
      .allowance(account, zeroExDeployedAddresses.erc20Proxy)
      .callAsync();
    const usdcAllowance = await usdcToken
      .allowance(account, zeroExDeployedAddresses.erc20Proxy)
      .callAsync();
    const usdcAllowanceText = Web3Wrapper.toUnitAmount(
      usdcAllowance,
      usdcDecimals.toNumber()
    ).decimalPlaces(2);
    const daiAllowanceText = Web3Wrapper.toUnitAmount(
      daiAllowance,
      daiDecimals.toNumber()
    ).decimalPlaces(2);

    setTextOnDOMElement(
      'fakeDaiBalance',
      Web3Wrapper.toUnitAmount(daiBalance, daiDecimals.toNumber())
        .decimalPlaces(2)
        .toString()
    );
    setTextOnDOMElement(
      'fakeUsdcBalance',
      Web3Wrapper.toUnitAmount(usdcBalance, usdcDecimals.toNumber())
        .decimalPlaces(2)
        .toString()
    );
